...
<body class="p-5">
    ...
    let currentTarget = null; // 'product', 'user', or 'nominee'

    // Handle "Choose" button click to remember which field is active
    ...
    $('#addImageBtn').on('click', function () {
        if (selectedImage && currentTarget) {
            ...
            // Update or create hidden input for image_id
            const hiddenFieldId = `${currentTarget}-image-hidden-id`;
            const group = $(`.imagegroup[data-target="${currentTarget}"]`);
            if ($(`#${hiddenFieldId}`).length === 0) {
                group.append(`<input type="hidden" id="${hiddenFieldId}" name="${currentTarget}_image_id" value="${selectedImage.id}">`);
            } else {
                $(`#${hiddenFieldId}`).val(selectedImage.id);
            }
    ...
    // Load images from backend and populate gallery
    function loadImages(sort = 'newest') {
        const searchQuery = $('#image-search').val().toLowerCase();

        $.get("{{ route('images.view') }}", { sort }, function (data) {
            ...
            // Filter images
            const filtered = data.filter(img => img.original_name.toLowerCase().includes(searchQuery));
    ...
    // Search & sort
    $('#image-search').on('input', () => loadImages($('#sort-select').val()));
    $('#sort-select').on('change', () => loadImages($('#sort-select').val()));
